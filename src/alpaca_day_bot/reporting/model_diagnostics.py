...
def model_diagnostics_for_day_by_action(
    db_path: str, day: date, *, action: str
) -> ModelDiagnostics | None:
    """
    Same as model_diagnostics_for_day but filters signals by action.
    Used for separate LONG vs SHORT diagnostics.
    """
    ...
    rows = conn.execute(
        ...
        FROM signals s
        JOIN forward_return_labels f ON f.signal_id = s.id
        WHERE s.ts BETWEEN ? AND ? AND s.action=?
        ORDER BY s.ts ASC
        """,
        (start, end, act),
    ).fetchall()
